...
public class AuthModelTreeNode {
    ...
    public void getChildren(List<SysAuthModel> allAuthModels) {
        if (ObjectUtils.isNull(authModelTreeNodeList)) {
            authModelTreeNodeList = new ArrayList<>();
        }
        ListIterator<SysAuthModel> sysAuthModelListIterator = allAuthModels.listIterator();
        while (sysAuthModelListIterator.hasNext()) {
            SysAuthModel next = sysAuthModelListIterator.next();
            //如果下一个是当前的子，就加入子节点
